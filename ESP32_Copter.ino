...
boolean armed = false;
uint8_t armct = 0;
int debugvalue = 0; //鑒察程式錯誤的代碼，初始為0
...
void setup() 
{
  ...
  WiFi.mode(WIFI_STA); // Station mode for esp-now WiFi操控數值
  ...
  #if defined externRC //應該是接收器是否有接收到數值，沒有再重試
    init_RC();
  #else
...
void loop() 
{
  ...
  if (recv)  //處理上面的recv數值
  {
    ...
    if (debugvalue == 4) Serial.printf("%4d %4d %4d %4d \n", rcValue[0], rcValue[1], rcValue[2], rcValue[3]);  //應為存取方向數值
  
    if      (rcValue[AU1] < 1300) flightmode = GYRO; //藉由參數調整飛行模式
